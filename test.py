...
class Forest:
    ...
    def plot_feature_importance(self, feature_names):
        indices = np.argsort(list(self.feature_importance.values()))[::-1]
        sorted_importance = np.array([self.feature_importance[i] for i in indices])
        ...
        plt.gca().invert_yaxis()  # 重要度从高到低显示
        plt.show()
